...
{
    public class OpenPayrollNotConnectedHandler : IConsumer<ProcessFlagRequestEvent>
    {
        private readonly ILogger<OpenPayrollNotConnectedHandler> _logger;
        private readonly IFlagHelper _flagHelper;

        public OpenPayrollNotConnectedHandler(
            ILogger<OpenPayrollNotConnectedHandler> logger,
        ...
        public async Task Consume(ConsumeContext<ProcessFlagRequestEvent> context)
        {
            ...
            if (!_flagHelper.IsValidVersion(context.Message, autodecisionComposite, FlagCode.OpenPayrollNotConnected, _logger)) return;
        ...
        {
            var response = _flagHelper.BuildFlagResponse(FlagCode.OpenPayrollNotConnected, autodecisionCompositeData, FlagResultEnum.Processed);
            try
            {
                const int maxConnectedDaysAccepted = 15;
                var shouldConnect = OpenConnectionsHelper.ShouldConnectOpenPayroll(autodecisionCompositeData.CreditPolicy.EmployerRules.EmployerRulesItems);
                if (!shouldConnect)
                {
            ...
            {
                _logger.LogError(e, $"FlagCode: {FlagCode.OpenPayrollNotConnected} was not successfully processed for LoanNumber: {autodecisionCompositeData.Application.LoanNumber} | Error: {e.Message}");
