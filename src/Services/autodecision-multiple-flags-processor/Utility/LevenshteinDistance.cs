namespace AutodecisionMultipleFlagsProcessor.Utility
{
    public class LevenshteinDistance
    {
        ...
        public static decimal CalculateSimilarity(string source, string target)
        {
            ...
            if ((source.Length == 0) || (target.Length == 0)) return 0m;
            if (source == target) return 1m;

            var levenshtein_distance = Compute(source, target);

            return Convert.ToDecimal(1.0 - (levenshtein_distance / (double)Math.Max(source.Length, target.Length)));
        }
    }
}
